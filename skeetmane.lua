...
if getgenv().Loaded and getgenv().Library then 
...
-- Library init
    ...
    local Fonts = {}; do
        ...
        local GamesenseIconFont = RegisterFont("GamesenseIcons", 400, "Normal", {
            Id = "gamesense_icons.ttf",
            Font = game:HttpGet("https://raw.githubusercontent.com/sh0tzik/assets/main/fonts/menu_font.ttf"),
        })

        Library.Font = Font.new(Verdana, Enum.FontWeight.Regular, Enum.FontStyle.Normal);
        Library.IconFont = Font.new(GamesenseIconFont, Enum.FontWeight.Regular, Enum.FontStyle.Normal);
    end
--
...
-- Library functions 
    ...
    -- Library element functions
        ...
        function Library:Tab(properties)
            local Cfg = {
                ...
                IconSize = properties.IconSize or 55,
                IconFont = properties.IconFont or Library.IconFont
            }
            Cfg.Index = #self.Tabs + 1
            ...
            local Items = Cfg.Items; do 
                -- Tab buttons 
                    ...
                    local IsImage = string.match(Cfg.Icon, "rbxassetid")
                    local IconProps = {
                        BorderColor3 = rgb(0, 0, 0);
                        Parent = Items.ButtonHolder;
                        Name = "\0";
                        Size = dim2(0, Cfg.IconSize, 0, Cfg.IconSize);
                        AnchorPoint = vec2(0.5, 0.5);
                        BackgroundTransparency = 1;
                    ...
                    if IsImage then
                        IconProps.ImageColor3 = rgb(100, 100, 100)
                        IconProps.Image = Cfg.Icon
                    else
                        IconProps.TextColor3 = rgb(100, 100, 100)
                        IconProps.Text = Cfg.Icon
                        IconProps.FontFace = Cfg.IconFont
                        IconProps.TextSize = Cfg.IconSize
                    end
                    Items.Icon = Library:Create(IsImage and "ImageLabel" or "TextLabel", IconProps)
                    ...
                    if IsImage then
                        ShadeProps.ImageColor3 = rgb(0, 0, 0)
                        ShadeProps.Image = Cfg.Icon
                    else
                        ShadeProps.TextColor3 = rgb(0, 0, 0)
                        ShadeProps.Text = Cfg.Icon
                        ShadeProps.FontFace = Cfg.IconFont
                        ShadeProps.TextSize = Cfg.IconSize
                    end
                    Items.Shade = Library:Create(IsImage and "ImageLabel" or "TextLabel", ShadeProps)
            ...
            function Cfg.OpenTab() 
                ...
                if Tab == Cfg.Items then return end
                
                local TargetIndex = Cfg.Index
                local CurrentIndex = self.CurrentTabIndex or 0
                
                if Tab then
                    local OldPage = Tab.Page
                    local Direction = 1
                    if TargetIndex < CurrentIndex then
                        Direction = -1
                    end
                    
                    local tween = Library:Tween(OldPage, {Position = dim2(0, 0, -Direction, 0)})
                    task.delay(Library.TweeningSpeed, function()
                        if self.TabInfo ~= Tab then
                            OldPage.Visible = false
                            OldPage.Parent = Library.Other
                        end
                    end)
                    ...
                    if Tab.Icon:IsA("ImageLabel") then
                        ...
                        Tab.Icon.TextColor3 = rgb(100, 100, 100)
                    end
                    Tab.Outline.Visible = false
                end

                if Items.Icon:IsA("ImageLabel") then
                    Items.Icon.ImageColor3 = rgb(255, 255, 255)
                else
                    Items.Icon.TextColor3 = rgb(255, 255, 255)
                end
                ...
                if Tab then
                    ...
                    Items.Page.Position = dim2(0, 0, Direction, 0)
                    Library:Tween(Items.Page, {Position = dim2(0, 0, 0, 0)})
                else
                    Items.Page.Position = dim2(0, 0, 0, 0)
                end
                
                self.TabInfo = Cfg.Items
                self.CurrentTabIndex = Cfg.Index
            end
        ...
        function Library:Toggle(properties) 
            ...
            local Items = Cfg.Items; do 
                ...
                Library:Create( "UIGradient" , {
                    Rotation = 90;
                    Parent = Items.Background;
                    Color = rgbseq{rgbkey(0, rgb(52, 52, 52)), rgbkey(1, rgb(36, 36, 36))}
                });                
            end;
        ...
        function Library:Slider(properties) 
            ...
            function Cfg.Set(value)
                ...
                Items.Accent.Size = dim2((Cfg.Value - Cfg.Min) / (Cfg.Max - Cfg.Min), 0, 1, 0)
        ...
        function Library:Listbox(properties) 
            local Cfg = {
                ...
                Options = properties.Options or {};
                Callback = properties.Callback or function() end;
                Scrolling = properties.Scrolling or true;

                OptionInstances = {};
                Items = {};
            }   

            Cfg.Default = properties.Default or Cfg.Options[1] or ""
            Flags[Cfg.Flag] = Cfg.Default
            ...
            local Items = Cfg.Items; do 
                ...
                Items.Outline = Library:Create( "Frame" , {
                    Parent = Items.Container;
                    Name = "\0";
                    Position = dim2(0, 20, 0, yOffset);
                    BorderColor3 = rgb(0, 0, 0);
                    Size = dim2(1, -55, 1, -yOffset);
                    BorderSizePixel = 1;
                    BackgroundColor3 = rgb(12, 12, 12)
                });
            ...
            function Cfg.RefreshOptions(options) 
                ...
                Items.Holder.CanvasSize = dim2(0, 0, 0, #options * 15)
            end
        ...
        function Library:PlayerListTree(properties)
            local Cfg = {
                Name = properties.Name or "Players";
                Flag = properties.Flag or properties.Name or "PlayerListTree";
                Callback = properties.Callback or function() end;

                Items = {};
                OptionInstances = {};
                CategoryInstances = {};
                CategoriesOpen = { Enemies = true, Teammates = true };
            }
            ...
            local Items = Cfg.Items; do 
                ...
                Items.SearchBoxOutline = Library:Create("Frame", {
                    Parent = Items.Container;
                    Name = "\0";
            ...
            local function RenderCategory(name, order)
                local CategoryBtn = Library:Create("TextButton", {
                    Parent = Items.Holder;
                    Size = dim2(1, 0, 0, 16);
                    BackgroundTransparency = 1;
                    Text = "";
                    LayoutOrder = order * 1000;
                });
                ...
                Cfg.CategoryInstances[name] = {Btn = CategoryBtn, Icon = Icon, Items = {}, BaseOrder = order * 1000}

                CategoryBtn.MouseButton1Down:Connect(function()
                    Cfg.CategoriesOpen[name] = not Cfg.CategoriesOpen[name]
                    Icon.Text = Cfg.CategoriesOpen[name] and "-" or "+"
                    Cfg.UpdateVisibility()
                end)
            end

            RenderCategory("Enemies", 1)
            RenderCategory("Teammates", 2)

            function Cfg.RenderOption(playerData)
                local Button = Library:Create("TextButton", {
                    Parent = Items.Holder;
                    Size = dim2(1, 0, 0, 16);
                    BackgroundTransparency = 1;
                ...
                local Dot = Library:Create("TextLabel", {
                    Parent = Button;
                    Position = dim2(0, 15, 0, 0);
                    Size = dim2(0, 15, 1, 0);
                    BackgroundTransparency = 1;
                    Text = "•";
                    TextColor3 = playerData.Color;
                    FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal);
                    TextSize = 18;
                ...
                table.insert(Cfg.OptionInstances, {
                    Button = Button,
                    NameLabel = NameLabel,
                    Data = playerData
                })
                table.insert(Cfg.CategoryInstances[playerData.Category].Items, Button)

                Button.MouseButton1Down:Connect(function()
                    Cfg.Set(playerData.Player)
                end)
            end

            function Cfg.UpdateVisibility()
                local search = Items.SearchBox.Text:lower()
                local yOffset = 0
                
                for _, cat in pairs(Cfg.CategoryInstances) do
                    cat.Btn.Visible = false
                end
                for _, opt in ipairs(Cfg.OptionInstances) do
                    opt.Button.Visible = false
                end

                for catName, catObj in pairs(Cfg.CategoryInstances) do
                    local hasVisibleItems = false
                    local itemOffset = 1
                    for i, optBtn in ipairs(catObj.Items) do
                        local optData
                        for _, o in ipairs(Cfg.OptionInstances) do
                            if o.Button == optBtn then optData = o break end
                        end
                        if optData and (search == "" or optData.Data.DisplayName:lower():find(search)) then
                            hasVisibleItems = true
                            if Cfg.CategoriesOpen[catName] then
                                optBtn.Visible = true
                                optBtn.LayoutOrder = catObj.BaseOrder + itemOffset
                    ...
                    if hasVisibleItems or search == "" then
                        catObj.Btn.Visible = true
                        yOffset = yOffset + 16
                    end
                end
                
                Items.Holder.CanvasSize = dim2(0, 0, 0, yOffset)
            end

            Items.SearchBox:GetPropertyChangedSignal("Text"):Connect(Cfg.UpdateVisibility)

            function Cfg.Set(player)
                for _, opt in ipairs(Cfg.OptionInstances) do
                    if opt.Data.Player == player then
                        opt.NameLabel.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal);
                        opt.NameLabel.TextColor3 = themes.preset.accent
                        opt.Button.BackgroundColor3 = rgb(26, 26, 26)
                        opt.Button.BackgroundTransparency = 0
                    else
                        opt.NameLabel.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal);
                        opt.NameLabel.TextColor3 = rgb(205, 205, 205)
            ...
            function Cfg.RefreshOptions(playersData)
                for _, opt in ipairs(Cfg.OptionInstances) do
                    opt.Button:Destroy()
                end
                Cfg.OptionInstances = {}
                for k, v in pairs(Cfg.CategoryInstances) do v.Items = {} end

                for _, pData in ipairs(playersData) do
                    Cfg.RenderOption(pData)
                end
                Cfg.UpdateVisibility()
            end
        ...
        function Library:PlayerCard(properties)
            local Cfg = {
                Items = {};
            }
            ...
            local Items = Cfg.Items; do 
                Items.Container = Library:Create("Frame", {
                    ...
                    Name = "\0";
                    BackgroundTransparency = 1;
                    Size = dim2(1, 0, 0, 100); -- Total height
                    BorderColor3 = rgb(0, 0, 0);
                    BorderSizePixel = 0;
                });
                
                Items.ImageOutline = Library:Create("Frame", {
                    Parent = Items.Container;
                    Name = "\0";
                ...
                Items.StatusLabel = Library:Create("TextLabel", {
                    Parent = Items.Container;
                    Name = "\0";
                    Position = dim2(0, 85, 0, 18);
                    Size = dim2(1, -85, 0, 15);
                    BackgroundTransparency = 1;
                    Text = "Online";
                    TextColor3 = rgb(144, 238, 144); -- Light Green
                    FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal);
                    TextSize = 13;
                    TextXAlignment = Enum.TextXAlignment.Left;
                });

                Items.IDLabel = Library:Create("TextLabel", {
                    Parent = Items.Container;
                    Name = "\0";
                    Position = dim2(0, 85, 0, 31);
                    Size = dim2(1, -110, 0, 15);
                    BackgroundTransparency = 1;
                    Text = "ID: 0";
                    TextColor3 = rgb(205, 205, 205);
                    FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal);
                ...
                Items.CopyID = Library:Create("ImageButton", {
                    Parent = Items.Container;
                    Name = "\0";
                    Position = dim2(1, -25, 0, 31);
                    Size = dim2(0, 14, 0, 14);
                    BackgroundTransparency = 1;
                    Image = "rbxassetid://83983589022863";
                    ImageColor3 = rgb(205, 205, 205);
                });
                Items.CopyID.MouseButton1Down:Connect(function()
                    if Cfg.Player then
                        if setclipboard then setclipboard(tostring(Cfg.Player.UserId)) end
                    end
                end)

                Items.AgeLabel = Library:Create("TextLabel", {
                    Parent = Items.Container;
                    Name = "\0";
                    Position = dim2(0, 85, 0, 44);
                    Size = dim2(1, -85, 0, 15);
                    BackgroundTransparency = 1;
                    Text = "Age: 0 days";
                    TextColor3 = rgb(205, 205, 205);
                ...
                Items.RatingLabel = Library:Create("TextLabel", {
                    Parent = Items.Container;
                    Name = "\0";
                    Position = dim2(0, 15, 0, 75);
                    Size = dim2(1, -15, 0, 30);
                    BackgroundTransparency = 1;
                    Text = "Premier rating: <font color=\"#6b8cff\">10918</font>\nWin: <font color=\"#90ee90\">+404</font>, Loss: <font color=\"#ff7f7f\">-117</font>, Tie: <font color=\"#ffff99\">+52</font>";
                    RichText = true;
                    TextColor3 = rgb(205, 205, 205);
                    FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal);
                    TextSize = 13;
            ...
            function Cfg.Set(player)
                Cfg.Player = player
                if not player then
                    Items.ImageLabel.Image = ""
                    Items.NameLabel.Text = "No player selected"
                    Items.StatusLabel.Text = ""
                    Items.IDLabel.Text = ""
                    Items.AgeLabel.Text = ""
                    Items.CopyID.Visible = false
                    return
                end
                
                Items.CopyID.Visible = true
                Items.ImageLabel.Image = "rbxthumb://type=AvatarHeadShot&id=" .. player.UserId .. "&w=150&h=150"
                Items.NameLabel.Text = player.Name
                Items.StatusLabel.Text = "Online"
                Items.IDLabel.Text = player.UserId
                
                local age = player.AccountAge
                if age > 365 then
                    Items.AgeLabel.Text = "[U:1:" .. math.floor(age / 365) .. "y " .. (age % 365) .. "d]"
                else
                    Items.AgeLabel.Text = "[U:1:" .. age .. "d]"
                end
            end 
        ...
        function Library:Textbox(properties) 
            ...
            local Items = Cfg.Items; do 
                ...
                if Cfg.Name then 
                    ...
                    Items.List.Size = dim2(1, 0, 0, 35)
                else
                    Items.List.Size = dim2(1, 0, 0, 20)
                end
            end 
        ...
        function Library:Keybind(properties) 
            ...
            Library:Connection(InputService.InputBegan, function(input, game_event) 
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    if not Library:Hovering(Items.KeybindOutline) then 
                        Cfg.SetVisible(false)
                        Cfg.Open = false;
                    end 
                end 
        ...
        function Library:Button(properties) 
            ...
            local Items = Cfg.Items; do 
                ...
                local gradient = Library:Create( "UIGradient" , {
                    Rotation = 90;
                    Parent = Items.Background;
                    Color = rgbseq{rgbkey(0, rgb(42, 42, 42)), rgbkey(1, rgb(26, 26, 26))}
                });
    ...
    -- Notification Library
        ...
        function Notifications:Create(properties)
            ...
            local parsedName = Cfg.Name
            local accentHex = "#" .. themes.preset.accent:ToHex()
            parsedName = parsedName:gsub("<accent>", '<font color="' .. accentHex .. '">')
            parsedName = parsedName:gsub("</accent>", "</font>")
